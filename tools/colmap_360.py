...
def pipeline(scene, base_path, n_views):
    ...
    res = os.popen( 'colmap feature_extractor --database_path database.db --image_path images  --SiftExtraction.use_gpu 1 --SiftExtraction.max_image_size 4032 --SiftExtraction.max_num_features 16384').read()
...
base_path = "./data/mipnerf360/"

for scene in ['bicycle', 'bonsai', 'counter', 'garden',  'kitchen', 'room', 'stump']:
    pipeline(scene, base_path = base_path, n_views = 12)  # please use absolute path!
    pipeline(scene, base_path = base_path, n_views = 24)  # please use absolute path!
