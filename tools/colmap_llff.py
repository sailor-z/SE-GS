...
def pipeline(scene, base_path, n_views):
    ...
    res = os.popen( 'colmap feature_extractor --database_path database.db --image_path images  --SiftExtraction.use_gpu 1 --SiftExtraction.max_image_size 4032 --SiftExtraction.max_num_features 32768').read()
    os.system('colmap exhaustive_matcher --database_path database.db --SiftMatching.use_gpu 1 --SiftMatching.guided_matching 1 --SiftMatching.max_num_matches 32768')
...
for scene in ['fern', 'flower', 'fortress',  'horns',  'leaves',  'orchids',  'room',  'trex']:
    pipeline(scene, base_path = base_path, n_views = 3)  # please use absolute path!
    pipeline(scene, base_path = base_path, n_views = 6)  # please use absolute path!
    pipeline(scene, base_path = base_path, n_views = 9)  # please use absolute path!
