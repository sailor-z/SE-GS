	...
	const bool debug)
{
  ...
  torch::Tensor tiles_touched = torch::full({P}, 0, means3D.options().dtype(torch::kUInt8));
  ...
  if(P != 0)
  {
    ...
    tiles_touched.contiguous().data<uint8_t>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, tiles_touched, radii, geomBuffer, binningBuffer, imgBuffer);
}
