...
def PILtoTorch_v2(pil_image, resolution):
    S = max(pil_image.size)
    if S > resolution:
        interp = PIL.Image.LANCZOS
    elif S <= resolution:
        interp = PIL.Image.BICUBIC
    new_size = tuple(int(round(x*resolution/S)) for x in pil_image.size)

    resized_image_PIL = pil_image.resize(new_size, interp)
    resized_image = torch.from_numpy(np.array(resized_image_PIL)) / 255.0
    if len(resized_image.shape) == 3:
        return resized_image.permute(2, 0, 1)
    else:
...
def get_expon_noise_ration_func(ratio_init, ratio_final, max_steps):
    def helper(step):
        t = np.clip(step / max_steps, 0, 1)
        ratio = np.exp(np.log(ratio_init) * (1 - t) + np.log(ratio_final) * t)
        return ratio
    return helper
