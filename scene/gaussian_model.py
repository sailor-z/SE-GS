...
class GaussianModel:
    ...
    def create_from_gs(self, gs_model, spatial_lr_scale: float):
        self.spatial_lr_scale = spatial_lr_scale
        fused_point_cloud = deepcopy(gs_model.get_xyz)
        ...
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._features_dc = deepcopy(gs_model._features_dc)
        self._features_dc = nn.Parameter(self._features_dc.requires_grad_(True))
        self._features_rest = deepcopy(gs_model._features_rest)
    ...
    def reset_attributes(self, reset_masks, min_scale=1.0, ratio=0.05):
        scale = torch.abs(self._xyz).mean(dim=0, keepdim=True).clamp(min_scale)
        new_xyz = self._xyz + torch.randn(self._xyz.shape, device=self._xyz.device) * \
        ratio * scale * reset_masks[:, None]

        scale = torch.abs(self._opacity).mean(dim=0, keepdim=True).clamp(min_scale)
        new_opacity = self._opacity + torch.randn(self._opacity.shape, device=self._opacity.device) * \
        ratio * scale * reset_masks[:, None]

        scale = torch.abs(self._scaling).mean(dim=0, keepdim=True).clamp(min_scale)
        new_scaling = self._scaling + torch.randn(self._scaling.shape, device=self._scaling.device) * \
        ratio * scale * reset_masks[:, None]

        new_rotation = matrix_to_rotation_6d(build_rotation(self._rotation))
        scale = torch.abs(new_rotation).mean(dim=0, keepdim=True).clamp(min_scale)
        new_rotation = new_rotation + torch.randn(new_rotation.shape, device=new_rotation.device) * \
        ratio * scale * reset_masks[:, None]
        new_rotation = rotation_6d_to_matrix(new_rotation)
        new_rotation = matrix_to_quaternion(new_rotation)
