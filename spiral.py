...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, near):
    ...
    output_path = os.path.join(Path(model_path).parent.parent, "videos")
    os.makedirs(output_path, exist_ok=True)

    os.system(f"ffmpeg -i " + render_path + f"/%5d.png -q 2 " + output_path + "/out_{}.mp4 -y".format(model_path.split('/')[-1]))
    os.system(f"ffmpeg -i " + render_path + f"/depth_%5d.png -q 2 " + output_path + "/out_depth_{}.mp4 -y".format(model_path.split('/')[-1]))
    os.system(f"ffmpeg -i " + render_path + f"/cdepth_%5d.png -q 2 " + output_path + "/out_cdepth_{}.mp4 -y".format(model_path.split('/')[-1]))

def render_sets(args, dataset : ModelParams, opt : OptimizationParams, iteration : int, pipeline : PipelineParams, near):
    with torch.no_grad():
        gaussians = GaussianModel(args)
        (model_params, _) = torch.load(os.path.join(dataset.model_path, "chkpnt10000.pth"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2000, 5000, 7000, 10000, 15000, 20000])
    ...
    render_sets(args, model.extract(args), op.extract(args), args.iteration, pipeline.extract(args), args.near)
